...
router:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 go build -o $(BUILD_DIR)/$(BINARY_NAME)-router-armv7 .
	# ARM 64 (e.g. newer ARM64 routers)
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o $(BUILD_DIR)/$(BINARY_NAME)-router-arm64 .
	# MIPSLE (Little Endian, e.g. MT7620/MT7621 based routers like Newifi D1, K2P)
	CGO_ENABLED=0 GOOS=linux GOARCH=mipsle GOMIPS=softfloat go build -o $(BUILD_DIR)/$(BINARY_NAME)-router-mipsle .
	# MIPS (Big Endian)
	CGO_ENABLED=0 GOOS=linux GOARCH=mips GOMIPS=softfloat go build -o $(BUILD_DIR)/$(BINARY_NAME)-router-mips .
